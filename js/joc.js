...
fetch("../utils/JSONPreguntes/preguntesEmissio.json")
  .then(res => {
    return res.json();
  })
...
fetch("../utils/JSONPreguntes/preguntesRecepcio.json")
  .then(res => {
    return res.json();
  })
...
fetch("../utils/JSONPreguntes/preguntesTransmissio.json")
  .then(res => {
    return res.json();
  })
...
novaPreguntaTransmissio = () => {
  if (
    llistaPreguntesTransmissio.length === 0 ||
    contPreguntes >= NUM_PREGUNTES
  ) {
    passarRanking();
    return window.location.assign("../views/final.html");
  }
...
opcions.forEach(opcio => {
  opcio.addEventListener("click", e => {
    ...
    const feedbackResposta =
      respostaEscollida == preguntaActual.resposta ? "correcte" : "incorrecte";
...
var preguntesEmissio = [
  "Pregunta emissió1",
  "Pregunta emissió2",
  "Pregunta emissió3"
];

var preguntesTransmissio = [
  "Pregunta transmissió1",
  "Pregunta transmissió2",
  "Pregunta transmissió3"
];

var preguntesRecepcio = [
  "Pregunta recepció1",
  "Pregunta recepció2",
  "Pregunta recepció3"
];
...
function passarRanking() {
  //Sumem la puntuació al jugador actual.
  var currentUser = getCookie("currentUser");
  currentUser = JSON.parse(currentUser);
  currentUser.puntuacio = parseInt(currentUser.puntuacio) + parseInt(punts);
  currentUser = JSON.stringify(currentUser);
  setCookie("currentUser", currentUser, 365);

  //Puntuació per mostrar a final.html
  setCookie("puntuacioQuiz", punts, 365);

  //Afegir jugador al ranking
  var arrayJugadors;
  var jugadorActual = getCookie("currentUser");
  jugadorActual = JSON.parse(jugadorActual);

  if (getCookie("jugadorsRanking") == "") {
    arrayJugadors = [];

    arrayJugadors.push(jugadorActual);
    arrayJugadors = JSON.stringify(arrayJugadors);
    setCookie("jugadorsRanking", arrayJugadors, 365);
  } else {
    arrayJugadors = getCookie("jugadorsRanking");
    arrayJugadors = JSON.parse(arrayJugadors);
    arrayJugadors.push(jugadorActual);
    arrayJugadors = JSON.stringify(arrayJugadors);
    setCookie("jugadorsRanking", arrayJugadors, 365);
  }
